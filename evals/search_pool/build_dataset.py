...
platforms=["노트북"])을 그대로 입력으로 써야 검색이 그 값을 어떻게 다루는지 볼 수 있다.
    ...
    .venv/bin/python evals/search_pool/build_dataset.py evals/agent_e2e/runs/<UTC timestamp>
"""
...
def main() -> None:
    ...
    for line in (args.run_dir / "results.jsonl").read_text(encoding="utf-8").splitlines():
        ...
        conditions = {k: v for k, v in record["conditions"].items() if k in SEARCH_FIELDS and v}
        cases.append(
            {
                "id": record["id"],
    ...
    payload = {"source_run": args.run_dir.name, "cases": cases}
