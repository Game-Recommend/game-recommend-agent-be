...
**정답의 근거는 `app/pipeline/query_processing/prompts.py`의 `QUERY_PARSER_SYSTEM`이다.**
문항마다 어느 절의 어떤 규칙인지 `basis`에 적었다. 프롬프트를 고치면 어느 문항이 흔들릴지
`basis`로 찾는다. 프롬프트가 정하지 않은 것은 정답으로 삼지 않았다.
