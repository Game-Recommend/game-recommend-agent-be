...
2026-09-17 측정. 같은 질문 5개를 원본 `game-recommend-be`(고정 파이프라인)와
이 저장소(에이전트)로 각각 한 번씩 실행한 기록입니다. 지연은 네트워크와 모델 상태에 따라 흔들립니다.

| 질문 유형 | 원본 | 에이전트 | 에이전트가 부른 Tool (순서) | 호출 수 |
| --- | --- | --- | --- | --- |
| 하드웨어 | 4.87초 · 0개 | 30.05초 · 3개 | 게임 검색 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 가격 | 5 |
| 멀티플레이 + 인원 | 17.48초 · 5개 | 27.07초 · 5개 | 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 리뷰 요약 | 6 |
| 가격 + 취향 | 22.47초 · 5개 | 28.76초 · 5개 | 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 | 5 |
| 플레이타임 + 장르 | 16.14초 · 5개 | 40.07초 · 5개 | 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 | 5 |
| 복합 조건 (데모용) | 24.77초 · 3개 | 19.71초 · 2개 | 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 리뷰 요약 | 6 |

### 읽는 법

- 같은 시각에 시작한 Tool 단계는 에이전트가 한 턴에 함께 부른 것이다(같은 턴 병렬 호출).
- `에이전트 추론`이 완료된 뒤에 나오는 Tool 단계는 러너 안전망이 직접 부른 것이다.
  추천 후보인데 가격·사양·리뷰를 조회하지 않은 게임이 있으면 러너가 대신 채운다.
...
- 원본 (4.87초): 추천 없음 · 경고 1건
  - 단계: 질문 분해 → 게임 검색 → 최종 답변 생성
- 에이전트 (30.05초): Thief II: The Metal Age, BioShock, BioShock 2
  - 단계: 질문 분해 → 에이전트 추론 → 게임 검색 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 가격 → 미디어

**멀티플레이 + 인원** — 친구 4명이서 온라인으로 같이 할 게임을 찾고 있어. 경쟁보다는 협동 위주였으면 좋겠고 한 판이 너무 길지 않았으면 좋겠어.

- 원본 (17.48초): Baldur's Gate II: Shadows of Amn, BioShock 2, Dead Space 2, League of Legends, Left 4 Dead 2 · 경고 3건
  - 단계: 질문 분해 → 게임 검색 → 가격 → 하드웨어 → 리뷰 요약 → 미디어 → 최종 답변 생성
- 에이전트 (27.07초): BioShock 2, Dead Space 2, League of Legends, Left 4 Dead 2, Star Wars: Jedi Knight - Dark Forces II · 경고 1건
  - 단계: 질문 분해 → 에이전트 추론 → 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 리뷰 요약 → 미디어

**가격 + 취향** — 지금 2만 원 이하로 살 수 있는 게임 중에서 스토리가 중요한 RPG 추천해줘. 턴제 게임은 별로 안 좋아해.

- 원본 (22.47초): Jade Empire: Special Edition, Vampire: The Masquerade - Bloodlines, Vampire: The Masquerade - Redemption, Fallout: A Post Nuclear Role Playing Game, Fallout 2
  - 단계: 질문 분해 → 게임 검색 → 가격 → 하드웨어 → 리뷰 요약 → 미디어 → 최종 답변 생성
- 에이전트 (28.76초): Jade Empire: Special Edition, Vampire: The Masquerade - Bloodlines, Vampire: The Masquerade - Redemption, Fallout 2, Fallout 3
  - 단계: 질문 분해 → 에이전트 추론 → 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 미디어

**플레이타임 + 장르** — 취업 준비하면서 가볍게 할 게임을 찾고 있어. 한 번에 30분~1시간 정도 하기 좋고, 전체 플레이타임도 15시간을 넘지 않는 싱글 게임이면 좋겠어.

- 원본 (16.14초): Max Payne 2: The Fall of Max Payne, BioShock 2, System Shock 2, Overlord: Raising Hell, Syndicate · 경고 3건
  - 단계: 질문 분해 → 게임 검색 → 가격 → 하드웨어 → 리뷰 요약 → 미디어 → 최종 답변 생성
- 에이전트 (40.07초): Max Payne 2: The Fall of Max Payne, BioShock 2, Overlord: Raising Hell, The Curse of Monkey Island, Portal 2
  - 단계: 질문 분해 → 에이전트 추론 → 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 미디어

**복합 조건 (데모용)** — RTX 3060, RAM 16GB PC를 사용하고 있어. 친구 한 명과 온라인으로 같이 할 수 있고, 공포 게임은 싫어. 3만 원 이하이면서 Steam 평가가 좋은 게임 3개만 추천해줘.

- 원본 (24.77초): Overlord II, Portal 2, Mass Effect 3
  - 단계: 질문 분해 → 게임 검색 → 가격 → 하드웨어 → 리뷰 요약 → 미디어 → 최종 답변 생성
- 에이전트 (19.71초): Overlord II, Mass Effect 3
  - 단계: 질문 분해 → 에이전트 추론 → 게임 검색 → 가격 → 하드웨어 → 리뷰 점수 → 리뷰 요약 → 리뷰 요약 → 미디어

### 이번 측정에서 본 차이

- **검색이 비면 고정 파이프라인은 거기서 끝난다.** 하드웨어 질문에서 원본은 충족 후보를 찾지 못해
  4.9초 만에 "추천할 게임이 없다"로 끝냈다. 에이전트는 같은 질문에서 검색 인자를 LLM이 직접 정해
  후보 30개를 얻고 Tool 5개를 부른 뒤 3개를 추천했다.
...
- **제외 목록은 에이전트가 얇다.** 복합 조건 질문에서 원본은 후보 전체를 조회해 20개를 제외 목록에
  남겼지만, 에이전트는 LLM이 고른 일부만 조회해 1개만 남겼다. 조회하지 않은 후보는 판단한 적이
  없으므로 제외 목록에 넣지 않는다는 `CandidateStore.checked()` 규칙 때문이다. FE의 "제외된 게임"
  카드가 원본보다 빈약해지는 대가다.
