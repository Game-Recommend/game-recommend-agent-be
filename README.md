...
위 예상 질문 5개를 원본 [game-recommend-be](https://github.com/Game-Recommend/game-recommend-be)
(고정 파이프라인)와 이 저장소(에이전트)로 각각 실행한 결과입니다. 원본은 2026-09-17, 에이전트는
2026-09-20의 측정이고, 원본 저장소의 앱 코드는 그 사이에 바뀌지 않았습니다. 질문별 상세·타임라인·원시
기록은 [evals/agent_questions/REPORT.md](evals/agent_questions/REPORT.md)에 있습니다.
...
- **도구 순서와 횟수가 질문마다 달라집니다.** 원본은 늘 같은 한 바퀴입니다. 에이전트는 평가를 물은 두
  질문(하드웨어, 복합 조건)에서만 리뷰 점수를 불렀고, 두 질문에서는 리뷰 요약을 부르지 않았습니다(그때는
  추천이 확정된 뒤 러너가 리뷰 요약을 채웁니다). 같은 턴에 고른 도구는 병렬로 실행합니다. 하드웨어
...
- **제외 목록은 비슷해졌습니다.** 가격·사양 Tool이 LLM이 고른 후보가 아니라 검색된 후보 전체를 조회하므로,
...
│  ├─ limits.py             Tool별 호출 상한과 이를 집행하는 미들웨어 ToolCallLimiter
